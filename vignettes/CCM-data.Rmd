...
library(LEANR)
library(doMC)
# by default doMC uses only half of the available cores
# to use more cores, add the number of cores as argument to 
# the following line, i.e. registerDoMC(3)
registerDoMC()
set.seed(123456)
...
LEAN_results<-lapply(names(CCM.pvals),function(ccm){
  run.lean.fromdata(CCM.pvals[[ccm]], g2, n_reps = 10000)
})
